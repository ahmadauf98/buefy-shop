          ...
          <v-col cols="9" class="mb-8">
            <v-card class="pa-5" flat>
              <p class="text-h5 font-weight-medium text-color-black">
                <strong>Edit</strong> Product Details
              </p>
    ...
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        firebase
          ...
          .onSnapshot((doc) => {
            if (doc.exists) {
              // Validation of product belong to seller
              if (doc.data().seller_id == this.seller_id) {
                // Get data of each product form
                this.name = doc.data().name
                this.description = doc.data().description
                this.price = doc.data().price
                this.sale_price = doc.data().sale_price
